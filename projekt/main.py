    ...
    def update(self):
        ...
        if not self.waiting_input:
            ...
            for button in self.pattern:
                button.animation(self, self.data["animation_speed"], self.data["fps"], self.clock)
            ...
            if self.clicked_button and self.buttons_player.index(self.clicked_button) == self.buttons_computer.index(self.pattern[self.step]):
                self.clicked_button.animation(self, self.data["animation_speed"], self.data["fps"], self.clock)
                self.step += 1
